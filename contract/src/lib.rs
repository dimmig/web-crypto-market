...
pub struct Market {
    version: u8,
    orders: UnorderedMap<String, TreeMap<OrderId, Order>>, // String: sell_token#buy_token, e.g NEAR#USDT
    order_id_to_order: LookupMap<OrderId, Order>, // not iterable; OrderId -> u64 for near-cli
...
impl FungibleTokenReceiver for Market {
    fn ft_on_transfer(
        ...
        env::log_str(&format!("Prefcessor id: {}", env::predecessor_account_id()));
        if msg.is_empty() {
            ...
            match message {
                TokenReceiverMessage::NewOrderAction {
                    ...
                    env::log_str(&format!("Creating new order action..."));
                    let order = self.add_order(new_order_action, sender_id.clone());
                    env::log_str(&format!("Order {} was created", order.0));

                    env::log_str(&format!("Matching order with id: {}", order.0));
                    self.match_order(sender_id, order.0, order.1.buy_amount, token);
...
impl Market {
    ...
    fn match_order(
        ...
        env::log_str(&format!("Order: {:?}", existed_order));
        ...
        ft_token::ft_transfer(
            order.maker, //galag.testnet
            order.buy_amount, // 1000
            "".to_string(),
            order.buy_token.clone(), // galag.testnet
            ONE_YOCTO,
            FT_TRANSFER_TGAS, // ft_transfer '{"receiver_id": "galag.testnet", "amount": "10000", "msg": ""}' --accountId galag.testnet --depositYocto 1 --gas ...
        )
    ...
    pub fn callback_on_send_tokens_to_maker(
        ...
        if is_promise_success {
            ...
            ft_token::ft_transfer(
                sender_id,
                U128(fee), // ft_transfer '{"receiver_id": ""}'
                "".to_string(),
                sell_token.clone(),
                ONE_YOCTO,
                FT_TRANSFER_TGAS,
    ...
    fn add_order(&mut self, action: NewOrderAction, sender: AccountId) -> (OrderId, Order) {
        let new_order = Order::from_action(action, sender);

        let key = compose_key(&new_order.sell_token, &new_order.buy_token); // NEAR#USDT
        let mut orders_map = self
            .orders
            .get(&key)
            .unwrap_or(TreeMap::new(key.as_bytes()));

        let order_id: OrderId = new_order.get_id();
    ...
    pub fn get_orders(
        ...
        let key = compose_key(&sell_token, &buy_token); // xabr.allbridge.testnet#abr.allbridge.testnet
        let order_by_key = self.orders.get(&key);
        ...
        // println!(
        //     "orders.contains_key: {}",
        //     orders.contains_key(&OrderId { key_price: 10000000000000000, hash: 11825599414792643000 };)
        // );
...
mod tests {
    ...
    fn test_add_order() {
        ...
        assert!(contract.get_pairs().len() == 0);
        assert!(contract.get_order(order_id_1.clone()).is_none());
    }
    ...
    fn test_orders_ordered_by_price() {
        ...
        let new_order_action_1 = NewOrderAction {
            ...
            sell_amount: U128(2), // 1000000000000000000000000000002
        ...
        let new_order_action_2 = NewOrderAction {
            sell_token: account1.clone(),
            sell_amount: U128(1), // 1000000000000000000000000000001
            buy_token: account2.clone(),
            buy_amount: U128(1),
        };
        ...
        let new_order_action_2 = NewOrderAction {
            sell_token: account1.clone(),
            sell_amount: U128(1),
            buy_token: account2.clone(), // 500000000000000000000000000000
            buy_amount: U128(2),
        };
        ...
        let new_order_action_3 = NewOrderAction {
            sell_token: AccountId::new_unchecked(String::from("dev-1653751184099-18920235830922")),
            sell_amount: U128(10000),
            buy_token: AccountId::new_unchecked(String::from("galag.testnet")),
            buy_amount: U128(1000),
        };

        contract.add_order(
            new_order_action_3,
            AccountId::new_unchecked(String::from("dgalagan.testnet")),
        );

        let unsorted_orders: Vec<_> = contract
            .get_orders(
                AccountId::new_unchecked(String::from("dev-1653751184099-18920235830922")),
                AccountId::new_unchecked(String::from("galag.testnet")),
            )
            .unwrap();

        env::log_str(&format!("Unsorted orders: {:?}", unsorted_orders));

        // let orders = contract
        //     .get_orders(account1, account2)
        //     .unwrap()
        //     .into_iter()
        //     .map(|i| i.order_id.key_price)
        //     .collect::<Vec<_>>();
    ...
    fn test_get_order_id_to_order() {
        ...
        testing_env!(builder
            .storage_usage(env::storage_usage())
            .attached_deposit(0)
            .predecessor_account_id(AccountId::new_unchecked(String::from("dgalagan.testnet")))
            .build());
        let account1: AccountId = "xabr.allbridge.testnet".parse().unwrap();
        let account2: AccountId = "abr.allbridge.testnet".parse().unwrap();
        ...
        let orders: Vec<_> = contract
            .get_orders(account1.clone(), account2.clone())
            .unwrap()
            .into_iter()
            .map(|i| i.order_id)
            .collect();

        let mut res: Vec<&OrderId> = vec![];
        ...
        env::log_str(&format!(
            "Order_id_to_order: {:?}",
            contract.order_id_to_order.get(&res[0])
        ));
